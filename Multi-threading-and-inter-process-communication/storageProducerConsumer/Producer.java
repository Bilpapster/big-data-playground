...
public class Producer extends Thread {
    ...
    public Producer(Server[] servers, int id) {
        this.servers = servers;
        this.id = id;
        this.runTime = 60000;
    }

    public Producer(Server[] servers, int id, long runTimeMilliseconds) {
        this.servers = servers;
        this.id = id;
    ...
    public void run() {
        ...
        while (System.currentTimeMillis() - startTime < this.runTime) {
            try {
                ...
                int valueToAdd = random.nextInt(91) + 10;
                try (Socket socket = new Socket("localhost", servers[serverIndex].getPort());
                     PrintWriter out = new PrintWriter(socket.getOutputStream(), true);
                     BufferedReader in = new BufferedReader(new InputStreamReader(socket.getInputStream()))) {

                    out.println("ADD");
                    out.println(valueToAdd);
                    String response = in.readLine();
                    System.out.println("Producer " + this.id + ": " + response + " on server " + serverIndex);
                }
